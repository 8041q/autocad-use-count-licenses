...
This filters the .log file by removing double and empty TIMESTAMPs leaving only the important ones, then saves two lists, one is the filtered and the other is with unique words (names, dates, timestamps), as so to make comparisons for counting uses and then saving it in an array (name, uses, month). It also then converts the date from american to european.  
